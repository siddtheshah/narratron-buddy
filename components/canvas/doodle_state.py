...
class DoodleState:
    ...
    def add(self, doodles: list[dict[str, object]]) -> None:
        if any(doodle.get("type") == "clear" for doodle in doodles): self.doodles.clear()
        else: self.doodles.extend(doodles)
        self._persist()
        ...
        try:
            ...
            for action in self.doodles:
                ...
                try:
                    ...
                    draw.line(points, fill=str(action.get("color", "#ffffff")),
